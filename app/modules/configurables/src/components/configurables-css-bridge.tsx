...
export function ConfigurablesCSSBridge() {
  ...
  useEffect(() => {
    ...
    if (isValidColor(brandColor.accent)) {
      ...
      root.style.setProperty("--navbar-background", brandColor.accent);
    }

    // ── Background ───────────────────────────────────────────────────────
    if (isValidColor((brandColor as any).background)) {
      root.style.setProperty("--background", (brandColor as any).background);
    }

    // ── Success / Alert ──────────────────────────────────────────────────
    if (isValidColor((brandColor as any).success)) {
      root.style.setProperty("--chart-2", (brandColor as any).success);
    }
    if (isValidColor((brandColor as any).alert)) {
      root.style.setProperty("--destructive", (brandColor as any).alert);
    }
  }, [config?.brandColor]);
